...
def orthogonal_encrypt(plain_text, step_size):
    ...
    for i in range(matrix_height):  # Mulai dari baris teratas
        matrix_row = []  # Baris matriks
        for j in range(matrix_width):
            index = (
                i + j * matrix_height
            )  # i = indeks baris saat ini, j = indeks kolom saat ini, matrix_height = jumlah baris
        ...
        if matrix_height % 2 == 0:
            if i % 2 == 0:
                for j in range(matrix_height):
                    if matrix_representation[j][i] != "@":
                        encrypted_text += matrix_representation[j][i]
                    else:
                        encrypted_text += "@"
            else:
                for j in range(matrix_height - 1, -1, -1):
                    if matrix_representation[j][i] != "@":
                        encrypted_text += matrix_representation[j][i]
                    else:
                        encrypted_text += "@"
